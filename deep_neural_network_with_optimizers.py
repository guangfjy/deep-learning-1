...
def DNN(X_train, y_train, X_test, y_test, layer_dims, learning_rate=0.0005, num_iterations=10000, optimizer='sgd', beta=0.9, beta2=0.999, mini_batch_size=64, epsilon=1e-8, records=None):
    parameters, costs = L_layer_model(X_train, y_train, layer_dims, learning_rate, num_iterations, optimizer, beta, beta2, mini_batch_size, epsilon)
...
def display_records(records):
    ...
    for record in records:
        ...
        print(f"length of cost: {len(costs)}, accuracy:{accuracy:.4f}")

        plt.plot(costs, label=f'{optimizer}-{accuracy:.4f}')
...
if __name__ == "__main__":
    ...
    # mini-batch
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, num_iterations=num_iterations)
    # momentum
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, num_iterations=num_iterations, optimizer='momentum')
    # nesterov momentum
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, learning_rate=0.0001, num_iterations=num_iterations, optimizer='nesterov_momentum')
    # adagrad
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, learning_rate=0.01, num_iterations=num_iterations, optimizer='adagrad')
    # adadelta
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, num_iterations=num_iterations, beta=0.9, epsilon=1e-6, optimizer='adadelta')
    # RMSprop
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, learning_rate=0.001, num_iterations=num_iterations, beta=0.9, epsilon=1e-6, optimizer='rmsprop')
    # adam
    DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], records=records, learning_rate=0.001, num_iterations=num_iterations, beta=0.9, beta2=0.999, epsilon=1e-8, optimizer='adam')
